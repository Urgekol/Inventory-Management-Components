document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSelectedColors(action, colorHex) 
  {
    const hexUpper = colorHex.toUpperCase();
    if (action === "add" && !selectedColors.includes(hexUpper)) 
    {
      selectedColors.push(hexUpper);
    } 
    else if (action === "remove") 
    {
      const index = selectedColors.indexOf(hexUpper);
      if (index !== -1) 
      {
